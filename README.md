...
1. In the Cloudflare dashboard, open the account's API tokens page and create a custom token with these permissions, all as Edit: `Account / Workers Scripts`, `Zone / Workers Routes`, `Zone / DNS`. Scope the account resource to this one account and the zone resource to the `zachsibert.com` zone. Copy the token; it is shown once.
